...
class FermiDiracDecoder(Module):
    def __init__(self, r, t=0.5):
        ...
        self.r = r
        self.t = max(t - 0.05, 0.05)  # Further decrease t for an even sharper transition
...
class HRCFModel(nn.Module):
    def __init__(self, users_items, args):
        ...
        self.embedding.state_dict()['weight'].uniform_(-args.scale * 0.1, args.scale * 0.1)
    ...
    def geometric_regularizer(self, embeddings):
        # Improved geometric regularizer: encourage embedding norms close to 1 and spread across the space.
        ...
        reg = ((item_norms - 1.0).pow(2).mean() + (user_norms - 1.0).pow(2).mean()) * 0.1
        ...
        pairwise_penalty = (torch.exp(-pairwise_distances).mean()) * 0.01
        return reg + pairwise_penalty
