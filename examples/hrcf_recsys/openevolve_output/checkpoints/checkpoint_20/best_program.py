...
# Optimize clamping for hyperbolic functions to reduce unnecessary operations
def cosh(x, clamp=10):
...
class FermiDiracDecoder(Module):
    def __init__(self, r, t, adaptive=False):
        super(FermiDiracDecoder, self).__init__()
        self.r = r
        self.t = t
        self.adaptive = adaptive

    def forward(self, dist, epoch=None):
        if self.adaptive and epoch is not None:
            r = self.r * (0.9 ** (epoch // 10))
            t = self.t * (1.1 ** (epoch // 10))
        else:
            r, t = self.r, self.t
        probs = 1. / (torch.exp((dist - r) / t) + 1)
        return probs
