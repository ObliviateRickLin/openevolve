...
def normalize(mx):
    """Normalize sparse matrix with identity."""
    return row_normalize(mx + sp.eye(mx.shape[0]))
...
class Hyperboloid(Manifold):
    ...
    def expmap(self, u, x, c):
        K = 1. / c
        sqrtK = K ** 0.5
        normu = self.minkowski_norm(u).clamp(max=self.max_norm)
        theta = normu / sqrtK
    ...
    def logmap(self, x, y, c):
        ...
        u = (y + xy * x * c).clone()
        normu = self.minkowski_norm(u).clamp(min=self.min_norm)
            ...
            for group in self.param_groups:
                ...
                for point in group["params"]:
                    ...
                    if momentum > 0:
                        ...
                        new_point = torch.cat([dim0, components], dim=1).contiguous()
...
class HRCFModel(nn.Module):
    def __init__(self, users_items, args):
        ...
        self.args = args
        self.dc = FermiDiracDecoder(r=args.r - 0.3, t=args.t + 0.05)
    ...
    def geometric_regularizer(self, embeddings, target_norm=1.0):
        ...
        user_mean_norm = ((1e-6 + user_embeddings.pow(2).sum(dim=1)).mean()).sqrt()
        item_mean_norm = ((1e-6 + item_embeddings.pow(2).sum(dim=1)).mean()).sqrt()
        user_reg = (user_mean_norm - target_norm).pow(2)
        item_reg = (item_mean_norm - target_norm).pow(2)
        return (user_reg + item_reg) / 2
    ...
    try:
        ...
        class Args:
            def __init__(self):
                ...
                self.lr = 0.0015  # Adaptive learning rate
                if epoch > 50 and avg_loss > previous_loss:
                    self.lr *= 1.05  # Slightly increase learning rate if no improvement
                self.momentum = 0.95
